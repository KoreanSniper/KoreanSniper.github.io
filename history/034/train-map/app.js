...
function updateVisibility() {
    ...
    lineElements.forEach(({ element, line }) => {
        const lineActive = isCategoryVisible(line.category);
        const defaultOpacity = line.planned ? (lineActive ? 0.9 : 0.18) : (lineActive ? 1 : 0.18);
        if (highlightedPath) {
            const isOnPath = element.getAttribute("stroke") === "#ef4444";
            element.setAttribute("stroke-opacity", isOnPath ? "1" : defaultOpacity);
        } else {
            element.setAttribute("stroke-opacity", defaultOpacity);
        }
    });
