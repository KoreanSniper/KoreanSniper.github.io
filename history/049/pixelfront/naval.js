...
export function installNaval(){
  ...
  Renderer.prototype.draw=function(){oldDraw.call(this);const e=this.e;e.ensureNaval?.();if(!e.navalMissions)return;const c=this.ctx,now=performance.now();
    for(const m of e.navalMissions){if(m.attacker!==0||m.state==="landed")continue;const[a,b]=e.xy(m.start),[x,y]=e.xy(m.target),t=m.state==="ready"?1:m.progress,from=this.point(a+.5,b+.5),to=this.point(x+.5,y+.5),q=this.point(a+(x-a)*t+.5,b+(y-b)*t+.5),angle=Math.atan2(to.y-from.y,to.x-from.x),size=Math.max(7,Math.min(13,q.s*2.1));c.save();c.strokeStyle="#42a5ff88";c.lineWidth=1.4;c.setLineDash([4,6]);c.lineDashOffset=-now*.02;c.beginPath();c.moveTo(from.x,from.y);c.lineTo(to.x,to.y);c.stroke();c.setLineDash([]);c.translate(q.x,q.y);c.rotate(angle);c.strokeStyle="#8fd0ff";c.lineWidth=1.5;c.fillStyle=m.state==="ready"?"#c8ff3d":"#42a5ff";c.beginPath();c.moveTo(size,0);c.lineTo(-size*.7,-size*.55);c.lineTo(-size*.38,0);c.lineTo(-size*.7,size*.55);c.closePath();c.fill();c.stroke();if(m.state==="sailing"){c.strokeStyle="#bce4ff99";c.lineWidth=1;c.beginPath();c.moveTo(-size*.65,-size*.32);c.lineTo(-size*1.15,-size*.55);c.moveTo(-size*.65,size*.32);c.lineTo(-size*1.15,size*.55);c.stroke()}c.restore();c.font="800 9px Inter";c.textAlign="center";c.lineWidth=3;c.strokeStyle="#061019";c.strokeText(short(m.troops),q.x,q.y-size-4);c.fillStyle=m.state==="ready"?"#c8ff3d":"#8fd0ff";c.fillText(short(m.troops),q.x,q.y-size-4)}
