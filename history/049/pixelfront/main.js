...
canvas.onpointerup=x=>{if(x.button===2)return;if(e.suppressNextTap){e.suppressNextTap=false;drag=false;return}drag=false;if(moved)return;const box=canvas.getBoundingClientRect(),i=r.tile(x.clientX-box.left,x.clientY-box.top);if(!e.running)deploy(i);else if(i>=0&&e.owner[i]!==0&&!r.visible[i]&&e.sharedBorder(0,e.owner[i]).length===0){toast("현재 확인되지 않은 지역입니다");return}else if(i>=0&&e.owner[i]!==0&&e.owner[i]!==-2){const target=e.owner[i],land=e.sharedBorder(0,target).length>0,landing=e.hasNavalReserve?.(0,target);if(!land&&!landing){toast("공격 경로를 확보할 수 없습니다");return}if(!e.issue({type:"attack",playerId:0,target:i,targetOwner:target,percent})){toast("현재 공격할 수 없습니다");return}toast(landing&&!land?"상륙 공격 명령 추가":"목표 국가 공격 명령 추가")}};
