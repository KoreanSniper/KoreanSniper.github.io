...
function applyAuthority(state){
  ...
  if(state.attacks){const localPlayerAttacks=e.attacks.filter(a=>a.attacker===0),synced=state.attacks.map(a=>({...a,attacker:gameServer.toLocalNation(a.attacker),defender:gameServer.toLocalNation(a.defender),front:new Set(a.front||[]),reinforcementQueue:a.reinforcementQueue||[]}));for(const local of localPlayerAttacks)if(!synced.some(a=>a.attacker===0&&a.defender===local.defender))synced.push(local);e.attacks=synced}
