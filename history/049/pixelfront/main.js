...
import{installNaval}from"./naval.js?v=6";
...
import{installMobileControls}from"./mobile-controls.js?v=2";
...
function applyAuthority(state,applyOwner=true){
  ...
  if(state.navalMissions){const routes=new Map((e.navalMissions||[]).map(m=>[m.id,m.route]));e.navalMissions=state.navalMissions.map(m=>({...m,route:m.route||routes.get(m.id),attacker:gameServer.toLocalNation(m.attacker),defender:gameServer.toLocalNation(m.defender)}))}
...
e.issue=command=>{if(command.playerId!==0)return issueLocal(command);if(!onlineSession){const accepted=issueLocal(command);if(!accepted)return false;if(authorityAvailable)gameServerReady.then(session=>session&&authorityAvailable&&gameServer.command(command)).catch(error=>{if(isAuthorityOutage(error)||error?.status===403||error?.status===409){authorityAvailable=false;console.info("PIXELFRONT authority mirror paused; local engine retained",error?.code||error?.status)}else console.info("PIXELFRONT local command was not mirrored to authority",error)});return true}const key=`${command.type}:${command.targetOwner??command.target??""}`;if(pendingCommands.has(key))return false;pendingCommands.add(key);gameServerReady.then(session=>{if(!session)throw new Error("SERVER_UNAVAILABLE");return gameServer.command(command).then(result=>{applyAuthority(result.state);ui();r.draw()})}).catch(error=>{toast(authorityMessage(error));if(error?.status===403)syncAuthority();else console.warn("PIXELFRONT command rejected",error)}).finally(()=>pendingCommands.delete(key));return true};
...
installMobileControls(canvas,e,r);
...
e.launchNaval=(playerId,tile,value)=>{if(playerId!==0||!authorityAvailable||!onlineSession)return navalLocal(playerId,tile,value);const target=e.resolveNavalTarget?.(playerId,tile)?.tile??tile;e.authorityCommand({type:"naval",tile:target,percent:value});return{ok:true,message:"상륙 명령 승인을 요청했습니다."}};
...
const runFrame=()=>{b.step();e.step();for(const nation of e.nations)if(nation.spawn>=0&&nation.tiles.size>0&&!nation.alive)nation.alive=true;ui();r.draw();const me=e.nations[0];if(!e.spectating&&!me.alive&&me.tiles.size===0){if(!defeatPendingAt)defeatPendingAt=performance.now();const elapsed=performance.now()-defeatPendingAt;if(!authorityAvailable||elapsed>=8000||elapsed>=3000&&!pendingCommands.size&&!syncBusy)finish(true)}else{defeatPendingAt=0;if(e.winner!==null)finish(false)}};
...
canvas.onpointerup=x=>{if(x.button===2)return;if(e.suppressNextTap){e.suppressNextTap=false;drag=false;return}drag=false;if(moved)return;const box=canvas.getBoundingClientRect(),i=r.tile(x.clientX-box.left,x.clientY-box.top);if(!e.running)deploy(i);else if(i>=0&&e.owner[i]!==0&&e.owner[i]!==-2){const target=e.owner[i];let adjacent=false;e.eachNeighbor(i,n=>{if(e.owner[n]===0)adjacent=true});if(!r.visible[i]&&!adjacent){toast("현재 확인되지 않은 지역입니다");return}const land=e.sharedBorder(0,target).length>0,landing=e.hasNavalReserve?.(0,target);if(!land&&!landing){toast("공격 경로를 확보할 수 없습니다");return}if(!e.issue({type:"attack",playerId:0,target:i,targetOwner:target,percent})){toast("현재 공격할 수 없습니다");return}toast(landing&&!land?"상륙 공격 명령 추가":"목표 국가 공격 명령 추가")}};
