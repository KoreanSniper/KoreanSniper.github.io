...
const incomingSeed = new URLSearchParams(location.search).get("seed");
...
form.addEventListener("submit", event => {
  ...
  params.set("name", String(data.get("name") || "플레이어").trim().slice(0, 14) || "플레이어");
  params.set("map", String(data.get("map") || "world"));
  params.set("ai", String(data.get("ai") || "8"));
  params.set("difficulty", String(data.get("difficulty") || "normal"));
  const seed = String(data.get("seed") || "").trim();
  if (seed) params.set("seed", String(Number(seed) >>> 0));
...
function connect(result) {
  ...
  unsubscribe = watchFirebaseRoom(result.code, room => {
    ...
    } else message(`${room.players?.length || 0}/8명 접속 중`);
