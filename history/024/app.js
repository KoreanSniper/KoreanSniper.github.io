...
function getLineCategory(lineName, line) {
    ...
    if (["JDX", "WSX", "YPX", "DDS", "DPX", "BSS", "OBR", "WDX"].includes(lineName)) return "high_speed";
...
function createLineBadge(lineName, stationName, direction, color, planned, category) {
    const station = stationMap[stationName],
        label = getBadgeLabel(lineName),
        fontSize = getBadgeFontSize(label),
        badgeOffset = 34,
        badgeRadius = 18,
        cx = station.x + direction.x * badgeOffset,
        cy = station.y + direction.y * badgeOffset,
    ...
    if (planned) circle.setAttribute("stroke-dasharray", "6 4");
    text.setAttribute("x", cx);
    text.setAttribute("y", cy + 2);
    text.setAttribute("text-anchor", "middle");
    text.setAttribute("dominant-baseline", "middle");
    text.setAttribute("class", "line-badge-text");
...
function applyLineFocus() {
    ...
    lineElements.forEach(({ element, line }) => {
        if (line.lineName === focusedLineName) {
            ...
            element.setAttribute("stroke-opacity", line.planned ? "0.22" : "0.15");
...
function clearHighlights() {
    ...
    lineElements.forEach(({ element, line }) => {
        ...
        element.setAttribute("stroke-opacity", line.planned ? "0.9" : "1");
...
function updateVisibility() {
    ...
    stationElements.forEach(({ name, element }) => {
        const visible = visibleStations.has(name);
        element.style.display = visible ? "" : "none";
        element.classList.toggle("dimmed", !visible);
        element.classList.toggle("highlighted", name === highlightedStation);
        element.style.opacity = highlightedPath && !pathStations.has(name) ? "0.2" : "1";
    });
    labelElements.forEach(({ name, element }) => {
        ...
        element.style.display = visible ? "" : "none";
        element.classList.toggle("dimmed", !visible);
        element.classList.toggle("compact", visible && !showLabel);
        element.style.opacity = highlightedPath && !pathStations.has(name) ? "0.2" : "1";
    });
...
function focusOnLine(name) {
    ...
    setResult(`<strong>${line.lineName}</strong><br>${categoryLabels[line.category]} / ${line.stations.length}개 역<br>${line.stations.join(" → ")}`);
    updateLineGroupState();
}
...
function renderLineGroups() {
    lineGroups.innerHTML = availableCategoryEntries.map(([key, label]) => {
        const groupLines = lineCatalog.filter(line => line.category === key),
            pills = groupLines.map(line =>
                `<div class="line-pill" data-line="${line.lineName}" data-category="${line.category}"><span class="swatch" style="background:${line.colorCss}"></span><span>${line.lineName}</span></div>`).join("");
        return `<section class="line-group"><h3>${label}</h3><div class="line-list">${pills || '<div class="line-pill">없음</div>'}</div></section>`;
    }).join("");
    lineGroups.querySelectorAll(".line-pill[data-line]").forEach(pill => {
...
renderedLines.forEach((line, index) => {
    const overlapGroup = overlapGroups[index],
        ...
        points = getRenderedSegmentPoints(line, overlapIndex, overlapGroup.length),
    ...
    lineElements.push({ element: el, line });
});
...
stationList.forEach(station => {
    ...
    circle.onmouseenter = () => circle.setAttribute("r", 14);
    circle.onmouseleave = () => circle.setAttribute("r", 10);
    circle.onclick = () => {
...
lineSearchInput.addEventListener("change", () => {
    const lineName = lineSearchInput.value.trim();
    if (!lineName) return;
    focusOnLine(lineName);
});
